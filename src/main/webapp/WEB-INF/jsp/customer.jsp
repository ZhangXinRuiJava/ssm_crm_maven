...
<title>客户列表-CRM</title>
	...
	<script type="text/javascript">
		...
		function editCustomer(id) {
			//alert('测试');
			$.ajax({
				url:"<%=basePath%>customerEdit.action",
				type:"post",
				data:{"id":id},
				dataType:'json',
		...
		function updateCustomer() {
			//alert("serialize:"+$("#edit_customer_form").serialize());
			$.post("<%=basePath%>customerUpdate.action",$("#edit_customer_form").serialize(),function(data){
				alert("客户信息更新成功！");
		...
		function deleteCustomer(id) {
			if(confirm('确实要删除该客户吗?')) {
				$.post("<%=basePath%>customerDelete.action",{"id":id},function(data){
					alert("客户删除更新成功！");
					window.location.reload();
				});
			}
			...
			<div class="panel panel-default">
				<div class="panel-body">
					<form class="form-inline"
						action="${pageContext.request.contextPath }/queryAllCustomer.action"
						method="post">
						<div class="form-group">
							<label for="customerName">客户名称</label> <input type="text"
								class="form-control" id="customerName" value="${param.custName }"
								name="custName">
						</div>
						<div class="form-group">
							<label for="customerFrom">客户来源</label> <select
								class="form-control" id="customerFrom" placeholder="客户来源" name="custSource">
								<option value="">--请选择--</option>
								<c:forEach items="${fromType}" var="item">
									<option value="${item.dictId}" <c:if test="${item.dictId == param.custSource}">selected</c:if> >
										${item.dictItemName }
									</option>
								</c:forEach>
							</select>
						...
						<div class="form-group">
							...
							<select	class="form-control" id="custIndustry"  name="custIndustry">
								...
								<c:forEach items="${industryType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == param.custIndustry}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custLevel" name="custLevel">
								...
								<c:forEach items="${levelType}" var="item">
									<option value="${item.dictId}"<c:if test="${item.dictId == param.custLevel}"> selected</c:if>>${item.dictItemName }</option>
								</c:forEach>
							</select>
						</div>
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						...
						<div class="col-md-12 text-right">
							<itcast:page url="${pageContext.request.contextPath }/queryAllCustomer.action"></itcast:page>
						</div>
						<!-- /.panel-body -->
					</div>
					<!-- /.panel -->
